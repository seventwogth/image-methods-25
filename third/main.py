...
def build_control_scenarios() -> list[ControlScenario]:
    ...
    return [
        ControlScenario(
            ...
            description="Зеркальная поверхность, наблюдатель расположен вдоль отраженного луча.",
            ...
            material_type="зеркальный",
            expected_behavior="Ожидается ненулевая яркость.",
            expect_nonzero=True,
        ),
        ControlScenario(
            scenario_id="observer_below_mirror",
            description="Наблюдатель находится под зеркальной поверхностью.",
            surface_point=origin_point,
            material=mirror_material,
            light=LightSource(position=Vec3(1.0, 0.0, 1.0), intensity=Vec3(1.0, 1.0, 1.0)),
            observer_position=Vec3(-2.0, 0.0, -2.0),
            material_type="зеркальный",
            expected_behavior="Яркость должна быть нулевой для наблюдателя с обратной стороны.",
            expect_nonzero=False,
        ),
        ControlScenario(
            scenario_id="light_below_surface",
            description="Источник света находится под односторонней поверхностью.",
            surface_point=origin_point,
            material=mirror_material,
            light=LightSource(position=Vec3(0.0, 0.0, -2.0), intensity=Vec3(1.0, 1.0, 1.0)),
            observer_position=Vec3(0.0, 0.0, 3.0),
            material_type="зеркальный",
            expected_behavior="Яркость должна быть нулевой, потому что свет приходит с обратной стороны.",
            expect_nonzero=False,
        ),
        ControlScenario(
            scenario_id="diffuse_front_lit",
            description="Диффузная поверхность освещена с лицевой стороны.",
            surface_point=origin_point,
            material=diffuse_material,
            light=LightSource(position=Vec3(0.0, 0.0, 2.0), intensity=Vec3(1.0, 1.0, 1.0)),
            observer_position=Vec3(0.0, 0.0, 3.0),
            material_type="диффузный",
            expected_behavior="Ожидается ненулевая диффузная яркость.",
            expect_nonzero=True,
        ),
        ControlScenario(
            scenario_id="diffuse_back_lit",
            description="Диффузная поверхность освещена с обратной стороны.",
            surface_point=origin_point,
            material=diffuse_material,
            light=LightSource(position=Vec3(0.0, 0.0, -2.0), intensity=Vec3(1.0, 1.0, 1.0)),
            observer_position=Vec3(0.0, 0.0, 3.0),
            material_type="диффузный",
            expected_behavior="Яркость должна быть нулевой, потому что поверхность освещена с обратной стороны.",
            expect_nonzero=False,
        ),
        ControlScenario(
            scenario_id="observer_back_side",
            description="Наблюдатель находится с обратной стороны освещенной односторонней поверхности.",
            surface_point=origin_point,
            material=diffuse_material,
            light=LightSource(position=Vec3(0.0, 0.0, 2.0), intensity=Vec3(1.0, 1.0, 1.0)),
            observer_position=Vec3(0.0, 0.0, -3.0),
